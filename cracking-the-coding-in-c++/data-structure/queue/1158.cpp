...
int main()
{
	...
	for (int i = 0; i < N; i++)
	{
		queue.push(i + 1); // 각 사람에게 1번부터 N번까지 번호 부여
	}
	...
	while (!queue.empty())
	{
		...
		if ((countOrder) == K)
		{
			cout << queue.front() << ", "; // K 번째 사람의 순번 출력

			queue.pop(); // queue의 맨 앞에 남아있는 순번의 값은 제거
		...
		{
			queue.push(queue.front()); // queue의 맨 뒤로 이동
			queue.pop(); // queue의 맨 앞에 남아있는 이동된 값 제거
